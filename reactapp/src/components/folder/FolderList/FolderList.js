...
function FolderList() {
    ...
    return (
            <div className={"flex flex-wrap gap-x-6 ml-6"}>
                {folders.map(folder => (
                        <button key={folder.id} className="bg-gray-200 p-2 rounded-lg flex items-center space-x-2 mt-4 w-32">
                            ...
                            <h3 className="text-slate-900 font-semibold">
                                <Link to={`/folder/detail/${folder.id}`}>{folder.name.length > 8 ? folder.name.slice(0, 8) + '...' : folder.name}</Link></h3>
                        </button>
                ))}
            </div>
    );
